...
async function main() {
  ...
  // Set of team slugs that actually have at least one fixture; matches the
  // skip condition in scripts/generate-team-feeds.mjs so we only advertise
  // <link rel="alternate"> for feeds that physically exist.
  const teamsWithFixtures = new Set();
  for (const f of fixtureRows) {
    for (const candidate of [safeStr(f?.home), safeStr(f?.away)]) {
      if (!candidate) continue;
      // Match against any known team label using the same fuzzy comparison
      // generate-team-feeds.mjs uses, then capture the slug for this team.
      for (const [slug, data] of teamMap.entries()) {
        if (teamMatch(data.label, candidate) || teamMatch(candidate, data.label)) {
          teamsWithFixtures.add(slug);
        }
      }
    }
  }
  ...
  for (const [slug, data] of teams) {
    ...
    // Only advertise the per-team RSS alternate when the feed actually
    // gets generated (generate-team-feeds.mjs skips teams with no fixtures).
    if (teamsWithFixtures.has(slug)) {
    ...
    assertNoPlaceholders(finalHtml, `teams/${slug}/index.html`);
    const h1 = (finalHtml.match(/<h1\b/gi) || []).length;
