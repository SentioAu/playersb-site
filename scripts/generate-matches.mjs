    ...
    <script>
      (function () {
        ...
        async function checkForUpdates(manual) {
          try {
            ...
            if (nextGeneratedAt && nextGeneratedAt !== initialGeneratedAt) {
              if (refreshLabel) {
                refreshLabel.textContent = "New match data detected. Refreshing view…";
              }
              track("matches_data_refresh", { source: manual ? "manual" : "auto", has_update: true });
              window.location.reload();
              return;
            }
            if (manual && refreshLabel) {
              refreshLabel.textContent = "No newer feed available yet. Checking again automatically.";
            }
            if (manual) {
              track("matches_data_refresh", { source: "manual", has_update: false });
            }
          } catch {
            if (manual && refreshLabel) {
        ...
        setInterval(() => {
          ...
          if (refreshLabel) {
            refreshLabel.textContent = "Auto-checking feed updates in " + secondsLeft + "s.";
          }
          if (secondsLeft === 60) {
